...
public class TimeCostMethodVisitor extends LocalVariablesSorter implements Opcodes {
    ...
    public TimeCostMethodVisitor(int access, String descriptor, MethodVisitor methodVisitor) {
        super(access, descriptor, methodVisitor);
    }

    protected TimeCostMethodVisitor(int api, int access, String descriptor, MethodVisitor methodVisitor) {
    ...
    public void visitInsn(int opcode) {
        if (opcode == RETURN) {
            ...
            mv.visitLdcInsn(200L);//阀值由TimeCostPlugin的扩展属性threshold控制
            mv.visitInsn(LCMP);
